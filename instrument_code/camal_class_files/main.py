...
def takeImage(site, telescope, imager, exptime, objname,filterlam=0):
    """
    ...
    # Take image
    logger.info("Taking " + str(exptime) + " sec image")
    imager.Expose(exptime, exptype, imager.filters[str(filterlam)])
    ...
    # Save the image
    filename = getIndex(imager.dataPath) + objname + "." + str(filterlam) + ".fits"
...
def loadSchedule(night):
    ...
    try:
        ...
        logger.info('No Schedule for Night %s Exists' %night)
        mail.send('No Schedule Found', 'Please make a schedule for night ' + night)
    lines = f.readlines()
    schedic  =  {}
    schedule =  []
...
def doScience(site, telescope, imager, targetinfo):
    ...
    # Slew to Target
    telescope.goto(ten(targetinfo['RA']),ten(targetinfo['DEC']))
    ...
    takeImage(site, telescope, imager, exptime823, targetinfo['name'],filterlam=823)
    takeImage(site, telescope, imager, exptime860, targetinfo['name'],filterlam=860)
    takeImage(site, telescope, imager, exptime780, targetinfo['name'],filterlam=780)

    # Calculate Center
    downloadSkyJPG(imager.dataPath)
    centerStar(imager,telescope,targetinfo)
    
    # Keep track of time
    timelastCenter = time.time()
    ...
    while datetime.datetime.utcnow() < targetinfo['endtime']:
        ...
        if time.time() - timelastCenter > 600:
            centerStar(imager,telescope,targetinfo)
            downloadSkyJPG(imager.dataPath)
            if not isDomeOpen():
                time.sleep(10)
            ...
            timelastCenter = time.time()
            logger.info('Centered Imaged')
            #centerStar()
        # Check if star is centered every 20min, if can't ifnd star check if dome is closed, else email and shut down
        #if not isDomeOpen: sleep(600)  # sleep for 10min if dome is closed


def centerStar(imager, telescope, targetinfo):
    """
    Load image, find star position, calculate dRA dDEC
    Return 1 if successful, -1 if can't find star & 0 if failed 
    """
    # Take last three images and median them
    files = glob.glob(imager.dataPath + '\\*' + targetinfo['name'] + '*.fits')
    lastthree = files[len(files)-3:]
 
    # Load data
    fall = []
    for filename in lastthree:
        f    = pyfits.open(filename)
        fall.append(f[0].data)
        hdr  = f[0].header
        f.close()
    # Take the median
    medf = np.median(np.array(fall),axis=0)
    hdr  = f[0].header

    # Telescope
    platescale = 0.71 * 2 #f[0].header['XBINNING'] #arcseconds/pixel  (bin by 2)
    RA, DEC = telescope.getRA(), telescope.getDEC()
    OBJCTRA, OBJCTDEC = ten(targetinfo['RA']),ten(targetinfo['DEC'])
    HA = telescope.tel.SiderealTime - RA
    
    # Determine Pier Side
    if HA >= 0: PS = 'west'
    if HA <  0: PS = 'east'

    # Find where object is in pixels
    x, y   = sourceFinder(medf)
    xcent, ycent = hdr['NAXIS1']/2, hdr['NAXIS2']/2
    dx, dy = xcent - x, ycent - y # positive == right, down
    #rashift, decshift = dx*platescale, dy*platescale #arcseconds

    # Don't Shift if didn't find source
    if x==0 or y==0:
        dx = 0
        dy = 0
        logging.info('Didnt Find Source so not moving telescope')
    # Move scope. what is rate?
    #rate = 7.5 #arcsec/sec? 
    rate = 100000/1000. #pixels/duration
    #duration = rate * distance
    durationx = abs(rate*dx)
    durationy = abs(rate*dy)
    # North South
    if PS == 'west':
        if dy < 0:   NS = 1
        if dy >= 0:  NS = 0
    ...
    #print durationx
    #print NS 
    telescope.tel.PulseGuide(NS,durationy)
    # East West
    telescope.tel.PulseGuide(EW,durationx)
    # telescope.MoveAxis(telescopeaxis, rate)
...
def updateReduction(night):
    ...
    """
    reductionfile = "C:/camal/analysis_code/" + 'reductionlist.txt'
...
#        if targetinfo['starttime']  < risetime:
#            targetinfo['starttime'] = risetime
#        if targetinfo['endtime']    > settime:
#            targetinfo['endtime']   = settime
#           ^^ this isn't working for me. nearest vs previous setting give next day sometimes :(
        if targetinfo['starttime'] < targetinfo['endtime']:
    ...
    # Check if we want to wait for these (put into a config file)
    dawnflats = False
    if dawnflats == True:
        ...
        #doSkyFlat(site, telescope, imager, flatFilters, num=CalibEndInfo['nflatEnd'], morning=True)
