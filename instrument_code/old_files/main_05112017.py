...
def centerStar(imager, telescope, targetinfo):
    ...
    rate0 = 3600*telescope.tel.GuideRateRightAscension # 3600 arcsec/deg * deg/second
    rate1 = 3600*telescope.tel.GuideRateDeclination
    #duration = rate * distance
    duration0 = abs(rate0*dx)
    duration1 = abs(rate1*dy)
    # North South
    if PS == 'west':
        if dy < 0:   NS = 1
        if dy >= 0:  NS = 0
    ...
    #print durationx
    #print NS 
    telescope.tel.PulseGuide(NS,duration0)
    # East West
    telescope.tel.PulseGuide(EW,duration1)
